...
class CI_Image_Uploader {
	...
	function __construct() {
		...
		if ( ! isset( $_POST['file_type'] ) ) {
			$this->response['error'] = esc_html__( 'Invalid File Type or size.', 'wp-user-avatar-pro' );
		}

		$this->file_type = $_POST['file_type'];

		if ( isset( $_POST['ci_crop_x'] ) && $_POST['ci_crop_x'] != '' ) {
			$this->crop_x = $_POST['ci_crop_x']; }

		if ( isset( $_POST['ci_crop_y'] ) && $_POST['ci_crop_y'] != '' ) {
			$this->crop_y = $_POST['ci_crop_y']; }

		if ( isset( $_POST['ci_crop_w'] ) && $_POST['ci_crop_w'] != '' ) {
			$this->crop_w = $_POST['ci_crop_w']; }

		if ( isset( $_POST['ci_crop_h'] ) && $_POST['ci_crop_h'] != '' ) {
	...
	function save_image( $width = 150, $height = 150 ) {
		...
		if ( $this->validate() ) {
			...
			if ( ! is_dir( $this->folder ) ) {
				wp_mkdir_p( $this->folder, $this->mkdir_mode, true );
			}
			...
			if ( isset( $this->file_name ) && $this->file_name != '' ) {
				...
				$filename = 'avatar' . time() . '.' . $this->file_type; }

			$target_path = $this->folder . '/' . $filename;

			if ( $this->file_type == 'jpeg' || $this->file_type == 'jpg' ) {
			...
			imagecopyresampled(
				$dst_r, $img_r, 0, 0, $this->crop_x, $this->crop_y,
