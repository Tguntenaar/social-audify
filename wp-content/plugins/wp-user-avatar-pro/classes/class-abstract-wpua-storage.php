...
abstract class Wpua_Storage {
	...
	protected function save() {
		...
		if ( count( $user_metadata ) < 4 ) {
			return new WP_Error( 'save_problem', esc_html__( 'Please set your storage meta properly', 'wp-user-avatar-pro' ) ); }

		$user_id = $this->user_id ? $this->user_id : get_current_user_id();
		if ( ! $user_id ) {
			return new WP_Error( 'save_problem', esc_html__( 'User not found.', 'wp-user-avatar-pro' ) ); }
