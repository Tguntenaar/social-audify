...
class S3StreamWrapper {
	...
	public function dir_opendir( $path, $options ) {
		...
		$this->file_list = $this->client( $protocol )->get_object_list(
			$bucket, array(
				'pcre' => $pattern,
			)
		);
	...
	public function rename( $path_from, $path_to ) {
		...
		$copy_response = $this->client( $protocol )->copy_object(
			array(
				...
				'filename' => $from_object_name,
			),
			array(
				'bucket' => $to_bucket_name,
				'filename' => $to_object_name,
			)
		);
	...
	public function stream_flush() {
		...
		$response = $this->client( $protocol )->create_object(
			$bucket, $object_name, array(
				'body' => $this->buffer,
			)
		);
	...
	public function stream_read( $count ) {
		...
		if ( $this->seek_position > 0 && $this->object_size ) {
			...
			$response = $this->client( $protocol )->get_object(
				$bucket, $object_name, array(
					'range' => $start . '-' . $end,
				)
			);
	...
	public function url_stat( $path, $flags ) {
		...
		if ( $object_name ) {
			$response = $this->client( $protocol )->list_objects(
				$bucket, array(
					'prefix' => $object_name,
				)
			);
