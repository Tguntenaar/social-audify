...
class CFRuntime {
		...
		if ( ! $credentials_provided ) {
			...
			if ( ! $this->credentials->default_cache_config ) {
				...
				throw new CFCredentials_Exception(
					'No credentials were provided. The SDK attempts to retrieve Instance '
	...
	public function cache_sts_credentials( $cache, $options ) {
		...
		// @codeCoverageIgnoreStart
		throw new STS_Exception(
			'Temporary credentials from the AWS Security '
	...
	public function cache_instance_profile_credentials( $cache, $options ) {
		...
		// @codeCoverageIgnoreStart
		throw new CFCredentials_Exception(
			'No credentials were provided. The SDK attempted to retrieve Instance '
	...
	public function authenticate( $operation, $payload ) {
		...
		if (
			...
			(integer) $request->get_response_code() === 503 ) {
	...
	private static function __getHash( $string, $secretk ) {
		return base64_encode(
			...
			hash_hmac( 'sha1', $string, $secretk, true ) : pack(
				'H*', sha1(
					( str_pad( self::$__secretKey, 64, chr( 0x00 ) ) ^ ( str_repeat( chr( 0x5c ), 64 ) ) ) .
					pack(
						'H*', sha1(
							( str_pad( $secretk, 64, chr( 0x00 ) ) ^
