...
?>		
	...
	<?php echo __('Your image files are currently using', 'wp-speedup').' <span></span> '.__('of your bandwidth!', 'wp-speedup'); ?> <?php echo (!isset($_GET['type'])?'<br /><a class="op_linked">('.__('Click here to optimize', 'wp-speedup').')</a>':''); ?></h3>
   <span><?php echo __('You can compress and try the compressed version of your images by switching directories.', 'wp-speedup'); ?></span>
    	<ul>
			...
			if(!$is_temp){
				...
				$dir_listing[] = '<li class="wpsu_link_dir '.(wpsu_temp(base64_decode($k))?'wpsu_temp_text':'').'" data-linked="'.$k.'"><span title="'.__('Directory Name', 'wp-speedup').': '.ucwords(base64_decode($k)).'">'.ucwords($dir_path).'</span> (<span>'.wpsu_formatSizeUnits($fs).'</span>) ';
				if($temp_path_exists): 
	...
	function wpsu_plugin_links($links) { 
		...
		if($su_pro){
			...
			$su_premium_link = '<a href="'.$su_premium_link.'" title="'.__('Go Premium', 'wp-speedup').'" target=_blank>'.__('Go Premium', 'wp-speedup').'</a>'; 
