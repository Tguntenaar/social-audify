...
/**
 * Image upload
 * https://www.isitwp.com/upload-user-submitted-files-frontend/
...
function prefix_register_api_routes() {
  ...
  $controller = new REST_controller($connection);
...
function send_test_mail()
{
  ...
  if ($_POST['mailcount'] == 0) {
    $subject = $user->subject_initial;
    $body = $user->initial_text;
  } else {
  ...
  if ($type == 'audit') {
    ...
    foreach ($audits as $audit) {
      if ($audit->view_time != NULL) {
        ...
        array_push($return_array, array(
          $audit->name, $audit->client_name, $audit->client_mail,
    ...
    foreach ($reports as $report) {
      if ($report->view_time != NULL) {
        ...
        $day_difference = $later->diff($earlier)->format("%a");
        array_push($return_array, array(
          $report->name, $report->client_name, $report->client_mail,
